...
target_time = datetime.datetime(
    year=current_time.year, month=current_time.month, day=current_time.day, hour=23, minute=55, second=0)
start_time = datetime.datetime(
    year=current_time.year, month=current_time.month, day=current_time.day, hour=00, minute=15, second=00)
...
def place_order(symbol, token, order_type, tokensp1, tokenbp1):
    if order_type == 'BUY':
        buy = shoon.MKT_BUY(str(symbol), 25)
        return token, symbol, buy, 'BUY', datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3], 1, tokensp1[str(token)]
    elif order_type == 'SELL':
        sell = shoon.MKT_SELL(str(symbol), 25)
        return token, symbol, sell, 'SELL', datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3], 1, tokenbp1[str(token)]
...
while start_time < datetime.datetime.now() < target_time:
    ...
    if aob.empty and count>5:
        if int(rounded_difference) > ENTRY:
            first_42_rows = token_data.head (int(entry_limit))
            next_42_rows = token_data.iloc[int(entry_limit):len(token_data)]
            ...
            if order_type == 'BUY':
                ...
                if buy[0] == 'COMPLETE':
                    ...
                    remaining_orders = [
                        (sorted_symbols[1], sorted_tokens[1], sorted_order_types[1]),
                        (sorted_symbols[2], sorted_tokens[2], sorted_order_types[2]),
                        (sorted_symbols[3], sorted_tokens[3], sorted_order_types[3])
                    ]

                    with concurrent.futures.ThreadPoolExecutor() as executor:
                        futures = []
                        for symbol, token, order_type in remaining_orders:
                            futures.append(executor.submit(place_order, symbol, token, order_type, tokensp1, tokenbp1))

                        for future in concurrent.futures.as_completed(futures):
                            result = future.result()
                            if result:
                                token, symbol, order_result, order_type, timestamp, status, price = result
                                aob.loc[len(aob)] = [token, symbol, order_result, order_type, timestamp, status, price]
                                ob.loc[len(ob)] = [token, symbol, order_result, order_type, timestamp, status, price]

                    save_lists('saved_lists.pkl', symbols_list, tokens_list, entry_point)
                    aob.to_csv(file_path_aob, index=False)
                    ob.to_csv(file_path_ob, index=False)

            # After the first sell order is complete
            elif order_type == 'SELL':
                ...
                if sell[0] == 'COMPLETE':
                    ...
                    aob.to_csv(file_path_aob, index=False)
                    ob.to_csv(file_path_ob, index=False)
                    # Prepare the remaining orders to be placed concurrently
                    remaining_orders = [
                        (sorted_symbols[1], sorted_tokens[1], sorted_order_types[1]),
                        (sorted_symbols[2], sorted_tokens[2], sorted_order_types[2]),
                        (sorted_symbols[3], sorted_tokens[3], sorted_order_types[3])
                    ...
                    with concurrent.futures.ThreadPoolExecutor() as executor:
                        futures = {executor.submit(place_order, symbol, token, order_type, tokensp1, tokenbp1): (symbol, token) for symbol, token, order_type in remaining_orders}

                        for future in concurrent.futures.as_completed(futures):
                            token, symbol, result, order_type, timestamp, status, price = future.result()
                            aob.loc[len(aob)] = [token, symbol, result, order_type, timestamp, status, price]
                            ob.loc[len(ob)] = [token, symbol, result, order_type, timestamp, status, price]
                list3 = [entry_point]
                save_lists('saved_lists.pkl', symbols_list, tokens_list, entry_point)
                aob.to_csv(file_path_aob, index=False)
                ob.to_csv(file_path_ob, index=False)
        ...
        if exit_difference > EXIT_DIFF:
            ...
            if order_type == 'BUY':
                ...
                if buy[0] == 'COMPLETE':
                    ...
                    # Place remaining orders concurrently
                    with concurrent.futures.ThreadPoolExecutor() as executor:
                        futures = {
                            executor.submit(place_order, symbol, token, order_type, tokensp1, tokenbp1): (symbol, token)
                            for symbol, token, order_type in remaining_orders   
                        }
                    ...
                    # Clear the aob dataframe and empty lists
